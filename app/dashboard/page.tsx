...
export default function Dashboard() {
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-5 pb-4">
      <section className="surface animate-fadeInScale p-5 sm:p-6 lg:p-7">
        <div className="grid gap-6 xl:grid-cols-[1.18fr_0.82fr] xl:items-start">
          <div>
            ...
            <h1 className="page-title-compact mt-4 max-w-3xl animate-fadeInUp [animation-delay:140ms] [animation-fill-mode:both]">
              {t.title}
            </h1>
            <p className="page-copy-compact mt-4">{t.description}</p>

            <div className="surface-muted mt-5 max-w-2xl p-4">
              <div className="flex items-start gap-3">
                <div className="icon-shell h-10 w-10 text-[var(--accent)]">
                  <Sparkles className="h-4 w-4" />
                </div>
                <div>
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">{t.quickTitle}</p>
                  <p className="mt-1 text-sm leading-6 text-slate-500 dark:text-slate-400">{t.quickBody}</p>
                </div>
              </div>
          ...
          <div className="grid gap-3 sm:grid-cols-3 xl:self-start">
            {quickStats.map(({ title, value, icon: Icon, progress }, index) => (
              ...
              >
                ...
                <p className="mt-3 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
                  {title}
                </p>
