...
export default function AuthShell({
  ...
  const contentAlign =
    align === 'start'
      ? 'items-start -mt-6 pt-0 sm:-mt-7'
      : 'items-center pt-3'

  return (
    <div className="relative grid h-dvh grid-rows-[auto_minmax(0,1fr)] overflow-hidden px-4 py-2 sm:px-6 lg:px-8">
      ...
      <div className="relative z-20 mx-auto flex w-full max-w-5xl items-center justify-between gap-3">
      ...
      <div className={`relative z-10 mx-auto flex min-h-0 w-full max-w-5xl justify-center ${contentAlign}`}>
        <section className={`surface w-full ${panelWidth} ${isCompact ? 'p-3.5 sm:p-4' : 'p-5 sm:p-6'}`}>
          <span className="eyebrow">{badge}</span>
          <h1 className={`max-w-2xl font-semibold tracking-tight text-slate-900 dark:text-white ${isCompact ? 'mt-1 text-[1.35rem] leading-tight sm:text-[1.5rem]' : 'mt-3 text-[1.7rem] leading-tight sm:text-[2rem]'}`}>
            {title}
          </h1>
